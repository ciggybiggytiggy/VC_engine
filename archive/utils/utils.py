...
@st.cache_data
def embed_portfolio(portfolio):

    names = list(portfolio.keys())
    desc = list(portfolio.values())
    vectors = model.encode(desc)

    return names, vectors

def semantic_overlap(df, portfolio):
    portfolio_names, port_vec = embed_portfolio(portfolio)
    portfolio_names = list(portfolio.keys())
    portfolio_desc = list(portfolio.values())

    port_vec = model.encode(portfolio_desc)
    start_vec = model.encode(df["description"].tolist())

    sim = cosine_similarity(start_vec, port_vec)

    df["portfolio_overlap_score"] = sim.max(axis=1)
    df["closest_portfolio_company"] = [portfolio_names[i] for i in sim.argmax(axis=1) if portfolio_names[i] != "Chicago Ventures"]
...
def competitor_overlap(df, competitor_portfolios):

    all_names = []
    all_desc = []
    comp_labels = []

    for comp, portfolio in competitor_portfolios.items():
        for name, desc in portfolio.items():
            all_names.append(name)
            all_desc.append(desc)
            comp_labels.append(comp)

    port_vec = model.encode(all_desc)
    start_vec = model.encode(df["description"].tolist())
    ...
    df["competitor_overlap_score"] = sim.max(axis=1)
    df["closest_competitor"] = [comp_labels[i] for i in best_idx]
...
def deal_flow_radar(df, portfolios):
    """
    Predict which VC fund is most likely to invest
    ...
    for _, row in df.iterrows():
        ...
        for fund_name, portfolio in portfolios.items():

            overlap = 0

            for company in portfolio.keys():
                if any(word in row["description"].lower() for word in company.lower().split()):
                    overlap += 1

            portfolio_score = overlap / max(len(portfolio), 1)

            if portfolio_score > best_score:
                best_score = portfolio_score
                best_fund = fund_name

        predicted_fund.append(best_fund)
        predicted_score.append(best_score)

    df["predicted_investor"] = predicted_fund
    df["deal_flow_score"] = predicted_score

    return df

def generate_deal_heatmap(df, portfolios):
    """
    Create a dataframe representing probability of each VC investing in each startup.
    """
    ...
    for _, row in df.iterrows():
        ...
        for fund_name, portfolio in portfolios.items():
            ...
            score = overlap / max(len(portfolio), 1)
